...
public class StoryAdapter extends RecyclerView.Adapter<StoryAdapter.ViewHolder>{
    ...
    public ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int i) {
        if (i==0){
            View view= LayoutInflater.from(mContext).inflate(R.layout.item_add_story,parent,false);
        ...
        reference.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                ...
                if (user.getImageurl().equals("noprofile")){
                    Glide.with(mContext).load(R.drawable.seedling_solid).into(viewHolder.story_photo);
                }else{
                ...
                if (pos!=0){
                    if (user.getImageurl().equals("noprofile")){
                        Glide.with(mContext).load(R.drawable.seedling_solid).into(viewHolder.story_photo_seen);
                    }else{
                        Glide.with(mContext).load(user.getImageurl()).into(viewHolder.story_photo_seen);
                    }
                    viewHolder.story_username.setText(user.getNickname());
                }
            }
